...
// Start the countdown phase - define first to avoid circular reference
...
// Create the handleVotingEnd function after startCountdown is defined
const handleVotingEnd = createHandleVotingEnd(rooms, broadcastRoomUpdate, startCountdown);

// Function to wrap startTopicVoting with dependencies
const startTopicVotingWithDeps = (roomId: string) => {
  startTopicVoting(roomId, rooms, broadcastRoomUpdate, handleVotingEnd);
};
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on('requestPlayAgain', () => {
    ...
    if (allWantToPlayAgain) {
        ...
        // --- INITIATE VOTING --- 
        startTopicVotingWithDeps(roomId);
    }
  });
